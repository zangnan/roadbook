...
11. **route_summary 字段必须使用"→"箭头连接各地点**，格式为"大理→丽江→香格里拉→大理"，不能使用自然语言描述；**相邻地点不能重复**，如"北京→北京→上海"应改为"北京→上海"
12. **fuel_price 油价字段必填，根据当前地区油价合理估算（如 8-9 元/升）**
13. **所有日期字段使用 yyyy-MM-dd 格式，如 2026-07-15**
14. **scenic_guides 景点攻略必须包含所有在 daily_itinerary.highlights 中出现的景点**，每个景点需要包含：
    - name: 景点名称（与 highlights 中一致）
    - location: 景点位置/地址
    - description: 100-200字的详细介绍
    - opening_hours: 开放时间
    - recommended_visit_time: 建议游览时长
    - ticket_info: 门票信息
    - tips: 2-3条游览贴士
15. **route_stops 详细行程节点**：每条 daily_itinerary 必须包含 route_stops 数组，记录当天的详细行程顺序，节点类型包括：
    - start: 出发地（type="start"，每组第一个节点）
    - scenic: 景点（type="scenic"）
    - accommodation: 住宿（type="accommodation"）
    - food: 餐饮（type="food"）
    - gas: 加油站（type="gas"）
    - transit: 中转/途经（type="transit"，location 应为具体地点如"XX服务区"、"XX市/县"，**不能用道路名称如"沈海高速"、"G304国道"**）
    - end: 终点/返程地（type="end"，**仅在最后一天的 route_stops 最后一个节点使用**）
    - 每个节点包含：name（UI显示文本）、location（标准地点名称，用于地图标注）、description（简短说明）
16. **避免"XX周边"与"XX"混用**：同一个城市不要一会儿写"锡林浩特市"一会儿写"锡林浩特市周边"，应统一使用标准城市名称（去掉"周边"后缀）
17. **相邻两天同一城市住宿时**，前一天的 accommodation.location 和后一天的 start.location 必须完全相同（如都是"霍林郭勒市"），不要一个写"霍林郭勒市"另一个写"霍林郭勒"或"霍林郭勒市某地"
18. **同一天内不要重复同一城市**：如果当天有 scenic 在某城市，别再用 accommodation 重复写该城市，保留 scenic 即可；例如：scenic "霍林郭勒市" + accommodation "霍林郭勒市" → 应去掉 accommodation 或改写其他地点"""
